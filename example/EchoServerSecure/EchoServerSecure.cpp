...
public:
    ...
    void onClosed(std::error_code ec) {
        LOG_INFO(_logger) << "Closed: " << ec << "(" << ec.message() << ")";
        _conn.takeover();
    }
protected:
...
int main (int argc, char **argv) {
    ...
    LOG_INFO(logger) << "Start listening on port: " << 22345;
    SslContextPtr context = SslContext::createDefaultServerContext("test.crt", "test.key");
    auto server = TcpServer::create(&ioLoop, std::move(context));
    server->bind(22345);
