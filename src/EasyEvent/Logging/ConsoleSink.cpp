...
void EasyEvent::ConsoleSink::setColor(LogLevel level) {
    ...
    fprintf((level < LOG_LEVEL_ERROR ? stdout : stderr), "\x1b[%d%sm", UnixColorFG[color],
            (color >= (uint8)ColorTypes::YELLOW && color < MaxColors ? ";1" : ""));
#endif
}
