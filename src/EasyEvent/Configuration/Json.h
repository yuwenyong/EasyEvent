...
#ifndef EASYEVENT_CONFIGURATION_JSON_H
...
namespace EasyEvent {
    ...
    class EASY_EVENT_API JsonValue {
    public:
        struct NullValue {
        ...
        using ValueType = std::variant<NullValue, int64_t, uint64_t, double, std::string, bool, ArrayType, ObjectType>;
        ...
        bool isNull() const {
            return isTypeOf(JsonType::NullValue);
        }

        bool isBool() const {
            return isTypeOf(JsonType::BoolValue);
        }
        ...
        bool isDouble() const {
            return isTypeOf(JsonType::IntValue) || isTypeOf(JsonType::UintValue) ||isTypeOf(JsonType::RealValue);
        }
        ...
        bool isString() const {
            return isTypeOf(JsonType::StringValue);
        }

        bool isArray() const {
            return isTypeOf(JsonType::ArrayValue);
        }

        bool isObject() const {
            return isTypeOf(JsonType::ObjectValue);
        }
    ...
    private:
        ValueType _value;
        std::array<std::string, CommentCount> _comments;
    };


    inline bool operator<(const JsonValue &lhs, const JsonValue &rhs) {
        if (lhs._value.index() != rhs._value.index()) {
            return lhs._value.index() < rhs._value.index();
        }
        VariantComparer<JsonValue::ValueType, VariantLessComp> visitor(lhs._value);
        return std::visit(visitor, rhs._value);
    }
    ...
    inline bool operator==(const JsonValue &lhs, const JsonValue &rhs) {
        ...
        VariantComparer<JsonValue::ValueType, VariantEqualComp> visitor(lhs._value);
        return std::visit(visitor, rhs._value);
    }
    ...
    class EASY_EVENT_API BuiltStyledStreamWriter: public StreamWriter {
    public:
        BuiltStyledStreamWriter(std::string indentation, CommentStyle cs, std::string colonSymbol,
                                std::string nullSymbol, std::string endingLineFeedSymbol, bool useSpecialFloats,
    ...
    private:
        ...
        void unindent() {
            Assert(_indentString.size() >= _indentation.size());
        ...
        std::vector<std::string> _childValues;
    ...
    class EASY_EVENT_API StreamWriterBuilder: public StreamWriter::Factory {
    public:
        StreamWriterBuilder();
    ...
    EASY_EVENT_API std::string writeString(const StreamWriter::Factory &factory, const JsonValue &root);

    EASY_EVENT_API std::ostream& operator<<(std::ostream &sout, const JsonValue &root);


    class EASY_EVENT_API CharReader {
    public:
    ...
    class BuiltReader: public NonCopyable {
    public:
    ...
    class EASY_EVENT_API BuiltCharReader: public CharReader {
    public:
        BuiltCharReader(bool collectComments, const ReaderFeatures &features)
            : _collectComments(collectComments), _reader(features) {
    ...
    class EASY_EVENT_API CharReaderBuilder: public CharReader::Factory {
    public:
        CharReaderBuilder();
    ...
    EASY_EVENT_API bool parseFromStream(const CharReader::Factory &factory, std::istream &sin, JsonValue* root,
                                        std::string *errs);

    EASY_EVENT_API std::istream& operator>>(std::istream &sin, JsonValue &root);

}

#endif //EASYEVENT_CONFIGURATION_JSON_H
